...
namespace ReservationApi.Infrastructure.Repositories
{
    public class BookingRepository(ReservationServiceDBContext context) : IBooking
    {
        public async Task<Response> CreateAsync(Booking entity)
        {
            try
            {
                var getBooking = await GetByAsync(p => p.BookingId!.Equals(entity.BookingId));
                var currentEntity = context.Bookings.Add(entity).Entity;
                await context.SaveChangesAsync();
                if (currentEntity is not null && currentEntity.BookingId.ToString().Length > 0)
                {
                    return new Response(true, $"{entity.BookingId} added to database successfully") { Data = currentEntity };
                }
                else
                {
                    return new Response(false, $"{entity.BookingId} cannot be added due to errors");
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<Booking> GetByIdAsync(Guid id)
        {
            try
            {
                var booking = await context.Bookings.AsNoTracking().SingleOrDefaultAsync(v => v.BookingId == id);
                return booking!;
            }
            catch (Exception ex)
            {
        ...
        public async Task<Response> UpdateAsync(Booking entity)
        {
            try
            {
                ...
                booking.TotalAmount = entity.TotalAmount == 0 ? booking.TotalAmount : entity.TotalAmount;
                booking.BookingDate = entity.BookingDate == default ? booking.BookingDate : entity.BookingDate;
                booking.Notes = string.IsNullOrEmpty(entity.Notes) ? booking.Notes : entity.Notes;
