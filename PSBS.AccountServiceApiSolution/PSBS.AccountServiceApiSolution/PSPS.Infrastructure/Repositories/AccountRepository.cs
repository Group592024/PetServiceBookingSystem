...
namespace PSPS.Infrastructure.Repositories
{
    ...
    internal class AccountRepository(PSPSDbContext context, IConfiguration config, IWebHostEnvironment _hostingEnvironment, IEmail _emailRepository) : IAccount
    {
        ...
        public async Task<GetAccountDTO?> GetAccount(string AccountGuId)
        {
            var account = await context.Accounts.FirstOrDefaultAsync(u => u.AccountGuId == AccountGuId);// Lấy tài khoản qua GUID
            if (account == null)
                return null;// Nếu không có tài khoản, trả về null

            return new GetAccountDTO(// Trả về DTO chứa thông tin tài khoản nếu tìm thấy
                account.AccountGuId ?? string.Empty,                             
        ...
        private string GenerateToken(Account account)       // Hàm tạo mã thông báo JWT cho tài khoản
    {
        ...
        var claims = new List<Claim>// Tạo danh sách claims để đính kèm thông tin tài khoản vào token
            ...
            if (!string.IsNullOrEmpty(account.RoleId) || !Equals("string", account.RoleId))
                ...
                if (model.UploadModel.ImageFile != null)// Kiểm tra và lưu ảnh đại diện người dùng nếu có
            {
                ...
                string filePath = Path.Combine(uploadPath, fileName);
                    ...
                    var newAccount = new Account()// Tạo tài khoản mới
                    {
                        ...
                        AccountGuId = Guid.NewGuid().ToString(), // Tạo GUID mới cho tài khoản
                ...
                return !string.IsNullOrEmpty(result.Entity.AccountGuId)
        ...
        public async Task<Response> UpdateAccount([FromForm] AddAccount model)// Cập nhật thông tin tài khoản người dùng
    {
            try
            {
                ...
                var account = await context.Accounts.FirstOrDefaultAsync(u => u.AccountGuId == model.AccountTempDTO.AccountGuId);
        ...
        public async Task<Response> ChangePassword(string accountGuId, ChangePasswordDTO changePasswordDTO)// Thay đổi mật khẩu người dùng
    {
            try
            {
            // Lấy thông tin tài khoản từ cơ sở dữ liệu
            var account = await context.Accounts.FirstOrDefaultAsync(a => a.AccountGuId == accountGuId);
