...
class _CustomerServiceBookingDetailState
  ...
  Future<void> fetchBookingDetails() async {
    try {
      final bookingResponse = await http.get(
        Uri.parse('http://127.0.0.1:5115/Bookings/${widget.bookingId}'),
      );
      ...
      final paymentResponse = await http.get(
        Uri.parse(
            'http://127.0.0.1:5115/api/PaymentType/${bookingData['paymentTypeId']}'),
      );
      setState(() {
      ...
      final accountResponse = await http.get(
        Uri.parse(
            'http://127.0.0.1:5000/api/Account?AccountId=${bookingData['accountId']}'),
      );
      setState(() {
        accountName =
      ...
      final statusResponse = await http.get(
        Uri.parse(
            'http://127.0.0.1:5115/api/BookingStatus/${bookingData['bookingStatusId']}'),
      );
      setState(() {
      ...
      final serviceItemsResponse = await http.get(
        Uri.parse(
            'http://127.0.0.1:5023/api/BookingServiceItems/${widget.bookingId}'),
      );
      ...
      if (serviceItemsData.isNotEmpty) {
        ...
        if (serviceVariantId != "00000000-0000-0000-0000-000000000000") {
          final serviceVariantResponse = await http.get(
            Uri.parse(
                'http://127.0.0.1:5023/api/ServiceVariant/$serviceVariantId'),
          );
          final serviceId =
              json.decode(serviceVariantResponse.body)['data']['serviceId'];
          if (serviceId != null) {
            final serviceResponse = await http.get(
              Uri.parse('http://127.0.0.1:5023/api/Service/$serviceId'),
            );
            setState(() {
      ...
      for (var item in serviceItemsData) {
        ...
        if (item['petId'] != null) {
          final petResponse = await http.get(
            Uri.parse('http://127.0.0.1:5010/api/Pet/${item['petId']}'),
          );
  ...
  Future<void> cancelBooking() async {
    ...
    final response = await http.put(
      Uri.parse('http://127.0.0.1:5115/Bookings/cancel/${widget.bookingId}'),
    );
