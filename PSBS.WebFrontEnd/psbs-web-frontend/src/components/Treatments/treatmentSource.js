...
const BASE_URL = 'http://localhost:5143/api/Treatment';

const Datatable = ({ treatments, onDetail, onEdit, onDelete }) => {
  ...
  const columns = [
    { field: "id", headerName: "ID", flex: 1, headerAlign: "center", align: "center", width: 200 },
    { field: "treatmentName", headerName: "Treatment Name", flex: 1, headerAlign: "center", align: "center", width: 50 },
    {
      field: "isDeleted",
      headerName: "Active",
      flex: 1,
      ...
      renderCell: (params) => {
        return (
          <div className="cellAction flex justify-around items-center w-full h-full">
            ...
            <button
              className="detailBtn"
              onClick={() => onDetail(params.row.treatmentId)}
            >
            ...
            <button
              className="editBtn"
              onClick={() => onEdit(params.row.treatmentId)}
            >
            ...
            <button
              className="deleteBtn"
              onClick={() => onDelete(params.row.treatmentId)}
            >
